...
public class Controller {
    ...
    public void run() {
        ...
        try {
            JsonNode csvConfig = configLoader.getCsvConfig();

            char delimiter = csvConfig.get("delimiter").asText().charAt(0);

            String bookArchivePath = csvConfig.get("bookArchiveFilePath").asText();
            String requestArchivePath = csvConfig.get("requestArchiveFilePath").asText();
            String orderArchivePath = csvConfig.get("orderArchiveFilePath").asText();

            String bookCatalogPath = csvConfig.get("bookCatalogFilePath").asText();
            String requestCatalogPath = csvConfig.get("requestCatalogFilePath").asText();
            String orderCatalogPath = csvConfig.get("orderCatalogFilePath").asText();

            saveManager = new SaveManager(delimiter,
                    bookArchivePath,
                    requestArchivePath,
                    orderArchivePath,
            ...
            loadManager = new LoadManager(delimiter,
                    bookArchivePath,
                    requestArchivePath,
                    orderArchivePath,
            ...
            throw new RuntimeException("Wrong config file: " + e.getMessage());
        }
